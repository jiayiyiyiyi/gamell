...
var MainLayer = cc.Layer.extend({
  ...
  conCom:function(){
    ...
    this.ruler.setPosition(83*fix, 196);
    ...
    this.lineOne.setPosition(97.3*fix, 196+34*fix);
  ...
  init : function(){
    ...
    this.leveEndDel.push(this.lineTwo);
    this.lineTwo.setAnchorPoint(0, 0);
    this.lineTwo.setPosition(data.lines[this.count].x*fix, 196+34*fix);
    ...
    this.bottle.setPosition(data.bottlePos[this.count].x*fix, data.bottlePos[this.count].y+40*fix);
   ...
   //空白回答
	...
	this.answerSpace.setPosition(340.7*fix, 40.3*fix);
    ...
    this.sun.setPosition(95, 25);
    this.answerSpace.addChild(this.sun, 5);
    ...
    this.question.setPosition(375*fix, 30.3*fix);
    ...
    this.answer.setPosition(340.7*fix, 40.3*fix);
    this.answer.id = 30;
        ...
        onTouchBegan: function (touch, event) {       //实现 onTouchBegan 事件处理回调函数
         ...
         onTouchEnded: function (touch, event) {            // 实现onTouchEnded事件处理回调函数                        
            ...
            if(target.id == 11){
				...
				if(this.count == 1){
					if(this.openArr.length >= 2){
						if(this.openArr[0]*10 + this.openArr[1] == 13){
							sound.rightAudio();
							/*移除所以监听事件*/
							cc.eventManager.removeAllListeners();
							var _this = cc.director.getRunningScene().getChildByTag(2);
