...
var Layer6 = cc.Layer.extend({
    ctor: function() {
        this._super();
        this.layerightStar= new StarLayer(common_data[5]);
    ...
    init:function () {
        this.frs = arrsubject[5];
        this.pos = arrsubject[5][3].slice(0);
        //埃及
        this.one = this.frs[0];
        this.france = new cc.Sprite(this.one[0].url);
        this.france.setAnchorPoint(0, 0);
        this.dealArr.push(this.france);
        this.france.setPosition(this.one[0].x*fix, this.one[0].y*fix+200/fix-200);
        this.france.setScale(0.333*fix);
        this.addChild(this.france, 0);
        ...
        if (common_data[5].obtain == 1){
            for(var i = 0;i<this.flagSix.length;i++){
                this.dragArr[i].removeFromParent();
                this.flagSix[i].removeFromParent();
            }
        }
    },
    dragAction : function(){
    ...
    judgeEach : function(target){
        for(var i=0;i<this.flagSix.length;i++){
            /*挑选相同 ID*/
            var s = this.flagSix[i].getBoundingBox();
            ...
            if (cc.rectIntersectsRect(rect1, rect)) {
                if(target.name === this.flagSix[i].name){
                    this.count++;
                    sound.rightAudio();
                    target.setOpacity(0);
                    this.flagSix[i].setOpacity(0);
                    this.moveFlag = false;
                    if(this.count == 3){
                        sound.franceAudio();
                        //出现飞星
